# -*- coding: utf-8 -*-
"""Correcciones_condicionales.ipynb

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1OLPnoRq9-Etsc4pf4l_bf_1yXSIwPxVL
"""
...
def validarCondicionalesPython(lineas):                                                                                       #Función para validar las estructuras que contengan condicionales dentro de un archivo de texto
    ...
    for line in lineas:                                                                                                 #Ciclo para leer el archivo de texto línea por línea
        ...
        #tabcont.append(calctab(line))

        sentence = line.strip('\t\n').strip().split(' ')                                                                        #Convierte cada línea del archivo de texto en un arraylist
        ...
        if 'SI' in sentence:
            ...
            if(result['aceptacion'] == 'Entra'):
                ...
                if RESULT[0]:
                    ...
                    for palabra in sentence:
                        ...
                        elif palabra in OPER.keys():
                            traduccion += (OPER[palabra])
                        else:
                            traduccion += (palabra+'') # bien
            ...
            if entro_si or conteo_si>0:
                ...
                if len(line) >= line.find('O')+1:                                                                                                  #Se evalua que se este dentro de una estructura SI, y se evalua que despues de la palabra SINO no se escriba mas nada si no que venga un salto de línea
                    for idx in range(line.find('O')+1, len(line)):
                        if line[idx] != ' ':
                            RESULT[0] = False
            ...
            if entro_sino:                                                                                                            
                ...
                if len(line) >= line.find('O')+1:
                    for idx in range(line.find('O')+1, len(line)):                                                                                 #Se valida que después del FSINO, no exista más nada, es decir que después de este vemga un salto de línea
                        if line[idx] != ' ':
                            RESULT[0] = False
            ...
            if entro_si:
                ...
                if len(line) >= line.find('I')+1:
                    for idx in range(line.find('I')+1, len(line)):                                                                               #Se valida que después del FSI, no exista más nada, es decir que después de este venga un salto de línea
                        if line[idx] != ' ':
                            RESULT[0] = False
            ...
            if entro_segun:
                ...
                if RESULT[0]:                                                                                               #Traducción a java 
                    ...
                    for palabra in sentence:
                        if palabra in TRADUCIR.keys():
                                traduccion += (TRADUCIR[palabra]+' ') # bien 
                        else:
                            traduccion += (palabra+'\n')
    ...
    #Al macenar los cambios que se hicieron a los case

    # modulo para realizar la tabulacion de la traduccion
    ...
    try:
      for i in range(len(RESULT[1])):
        ...
        sens = RESULT[1][i].strip('\t\n')

        if 'if' in sens:
          RESULT[1][i] = sacartabs(tabcont[i]+contab+contsw)+sens+'\n'
          contab += 1
        elif '|' in sens:
          RESULT[1][i] = ''
          contab -= 1
          ...
          if contsw > 0:
            RESULT[1][i] = sacartabs(tabcont[i]+contab-1+contsw)+sens+'\n'
          else:
            RESULT[1][i] = sacartabs(tabcont[i]+contab-1)+sens+'\n'
        elif 'match' in sens:
          RESULT[1][i] = sacartabs(tabcont[i]+contab)+sens+'\n'
          contab += 1
          contsw += 1
        elif 'case' in sens:
          RESULT[1][i] = sacartabs(tabcont[i]+contab)+sens+'\n'
